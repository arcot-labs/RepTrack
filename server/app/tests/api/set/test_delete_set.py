...
async def _make_request(
    ...
    return await make_http_request(
        ...
        endpoint=f"/api/sets/{workout_id}/exercises/{workout_exercise_id}/sets/{set_id}",
    )
...
# 401
async def test_delete_set_not_logged_in(client: AsyncClient):
    resp = await _make_request(
        client,
        workout_id=1,
...
# 404
async def test_delete_set_workout_not_found(
    client: AsyncClient,
    settings: Settings,
):
    await login_admin(client, settings)
