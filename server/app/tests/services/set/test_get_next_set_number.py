...
async def test_get_next_set_number_empty(session: AsyncSession):
    ...
    set_number = await _get_next_set_number(workout_exercise.id, session)
    assert set_number == 1


async def test_get_next_set_number_max(session: AsyncSession):
    user = await create_user(session)
    workout = await create_workout(session, user_id=user.id)
    exercise = await create_exercise(session, name="Squat")
    workout_exercise = await create_workout_exercise(
