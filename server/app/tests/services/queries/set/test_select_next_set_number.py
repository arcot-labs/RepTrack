...
from app.services.queries.set import (
    select_next_set_number,
)
...
async def test_select_next_set_number_empty(db_session: AsyncSession):
    ...
    set_number = await select_next_set_number(db_session, workout_exercise.id)
    assert set_number == 1


async def test_select_next_set_number_max(db_session: AsyncSession):
    user = await create_user(db_session)
    workout = await create_workout(db_session, user_id=user.id)
    exercise = await create_exercise(db_session, name="Squat")
    workout_exercise = await create_workout_exercise(
